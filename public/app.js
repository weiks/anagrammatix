  ...
  (function($) {
    ...
    var IO = {
      ...
      showLeader: function(data) {
        ...
        var table =
          '<div id="tablearea"><table id="leadertable"><tr><th>Player Name</th><th>Total Win</th></tr>'
        console.log('Showing Leader')
        var i = Object.keys(data).length
        for (var j = 0; j < i; j++) {
          //console.log(i);
          table +=
            '<tr><td>' + data[j].name + '</td><td>' + data[j].win + '</td></tr>'
        }
        table += '</table></div>'
        table +=
          "<div id='mid'><button id='back' class='btn'>BACK</button></div>"
        console.log(table)
      ...
      showWinner: function(txId) {
        console.log(txId)
        $('#hostWord').append(
          "<p><a href='https://ropsten.etherscan.io/tx/" +
            txId +
            "'>" +
            txId +
            '</a></p>'
